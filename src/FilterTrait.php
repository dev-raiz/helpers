...
trait FilterTrait
{
    ...
    public function filterCep(string $cep): string
    {
        $filter = str_replace('.', '', $cep);
        $filter = str_replace('-', '', $filter);
    ...
    public function filterMathAlphanumericCharacters($text)
    {
        $conversionTable = [
            '𝑨' => 'A',
            '𝑩' => 'B',
        ...
        $outputText = strtr($text, $conversionTable);
        return $outputText;
    }

    public function filterAccents(string $text): string
    {
        $accents   = array('à', 'á', 'â', 'ã', 'ä', 'å', 'ç', 'è', 'é', 'ê', 'ë', 'ì', 'í', 'î', 'ï', 'ñ', 'ò', 'ó', 'ô', 'õ', 'ö', 'ù', 'ü', 'ú', 'ÿ', 'À', 'Á', 'Â', 'Ã', 'Ä', 'Å', 'Ç', 'È', 'É', 'Ê', 'Ë', 'Ì', 'Í', 'Î', 'Ï', 'Ñ', 'Ò', 'Ó', 'Ô', 'Õ', 'Ö', 'O', 'Ù', 'Ü', 'Ú');
        $noAccents = array('a', 'a', 'a', 'a', 'a', 'a', 'c', 'e', 'e', 'e', 'e', 'i', 'i', 'i', 'i', 'n', 'o', 'o', 'o', 'o', 'o', 'u', 'u', 'u', 'y', 'A', 'A', 'A', 'A', 'A', 'A', 'C', 'E', 'E', 'E', 'E', 'I', 'I', 'I', 'I', 'N', 'O', 'O', 'O', 'O', 'O', 'O', 'U', 'U', 'U');

        $outputText = str_replace($accents, $noAccents, $text);
        return $outputText;
    }

    public function filterSymbols(string $text): string
    {
        $symbols   = array('”', '"', '!', '@', '#', '$', '%', '&', '*', '(', ')', '_', '-', '+', '=', '{', '[', '}', ']', '/', '?', ';', ':', '.', ',', '<', '>', '°', 'º', 'ª', '\\', '~');
        $noSymbols = array('', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '-', '', '', '', '', '', '', '', '', '', '', '', '');

        $outputText = str_replace($symbols, $noSymbols, $text);
        return $outputText;
    }

    public function filterEmoji(string $text): string
    {
    ...
    public function filterUnicode(string $text): string
    {
        $default = '/\[[Uu]\+[0-9A-Fa-f]+\]/';
        return preg_replace($default, '', $text);
    }
